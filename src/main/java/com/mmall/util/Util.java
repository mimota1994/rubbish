...
//        String sig =  com.sun.org.apache.xerces.internal.impl.dv.util.Base64.encode(mdInst.digest(paramB));
     ...
     */
    public static BodyObj resolveBody(Object body, boolean encrypted) {
//        JSONObject jsonMsg = new JSONObject(body);
        HashMap hashMsg = (HashMap)body;
        BodyObj obj = new BodyObj();
        obj.setNonce((String)hashMsg.get("nonce"));
        obj.setMsgSignature((String)hashMsg.get("msg_signature"));
        if (encrypted) {
            if (!hashMsg.containsKey("enc_msg")) {
                return null;
            }
            obj.setMsg(hashMsg.get("enc_msg"));
        } else {
            if (!hashMsg.containsKey("msg")) {
                return null;
            }
            obj.setMsg(hashMsg.get("msg"));
        }
        return obj;
    }
